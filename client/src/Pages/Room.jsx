...
function Room() {
	...
	const [randomRoomId, setRandomRoomId] = useState(0)
	...
	useEffect(() => {
		...
		const handleRoomIdGenerated = ({ result, random }) => {
			console.log(result, random)
			setRandomRoomId(random)
			setLoader(false)
		}
	...
	const handleDeductAmt = async () => {
		...
		try {
			...
			const depositTx = await poolContract.methods.deposit(randomRoomId, amountInWei).send({ from: fromAddress });
