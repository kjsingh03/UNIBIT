	...
	return (
		<div className='flex flex-col gap-8 pt-32'>
			<div className="flex flex-col lg:flex-row items-center lg:items-start justify-center gap-8 w-[95%] 2xl:w-[80%] mx-auto lg:h-[80vh] 2xl:h-[75vh] ">
				<div className={`flex flex-col items-center gap-6 lg:py-12 ${(!users.length > 0 || !joinedRoom) ? 'w-full' : 'w-max'} ${joinedRoom ? 'border' : ''} transition-all ease-in duration-300 p-4 rounded-xl ${users.length > 0 && users.filter(user => user.id === socketRef?.current?.id)[0]?.state ? 'border-green-600' : 'border-red-600'}`}>
					<div className="w-64">
